...
class GAT(torch.nn.Module):
    def __init__(self, num_node_features, num_classes):
        ...
        self.conv2 = GATConv(in_channels=3 * 16, out_channels=16, heads=2)
        self.conv3 = GATConv(in_channels=2 * 16, out_channels=num_classes, heads=1)

        self.dropout = nn.Dropout(0.2)
        self.topk1 = tnn.TopKPooling(in_channels=num_classes, ratio=0.5)
...
def evaluate_accuracy(
    ...
    with torch.no_grad():
        for batch in data_iter:
            ...
            net.eval()
            X,_=net(X.to(device))
            acc_sum += (X.argmax(dim=1) == y.to(device)).float().sum().item()
...
print(torch.topk(key_electrodes,10).indices)
